...
Handy Tools is a single Go module that produces four binaries from one core:
...
All four depend on `internal/tools/<feature>/` (image, archive, pdf, hash, difftree, rename, ...), which is the **only** layer allowed to touch files, run external binaries, or know about formats. `cmd/htools/`, `cmd/htools-mcp/`, `internal/server/`, and `internal/api/http/` are thin adapters — never put tool logic in any of them. The MCP server reuses the same `internal/server/*Handler` types as the gRPC/HTTP transports. See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).
...
make build      # bin/htools + bin/htoolsd + bin/htools-mcp
